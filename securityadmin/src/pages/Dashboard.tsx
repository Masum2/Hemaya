import type { ChildStat } from '@/types/dashboard';
import {
    Users,
    ShieldAlert,
    FileCheck,
        ...
        <div className="space-y-8 p-6">
            ...
            <div className="flex justify-between items-end">
                <div>
                    ...
                    <p className="text-slate-500 dark:text-slate-400 mt-1 font-medium">
                        Welcome back, Hasan. There are <span className="text-rose-500 font-bold underline">12 high-risk alerts</span> requiring your attention.
                    </p>
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {stats.map((stat, index) => (
                    <div key={index} className="p-6 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/60 dark:border-slate-800/60 shadow-sm hover:shadow-xl transition-all group">
                        <div className="flex justify-between items-start">
                            <div className={`p-3 rounded-2xl ${stat.bg} group-hover:scale-110 transition-transform`}>
                                <stat.icon className={stat.color} size={24} />
                            </div>
                            <span className={`flex items-center text-[10px] font-bold px-2 py-1 rounded-full ${stat.trendUp ? 'text-emerald-600 bg-emerald-50' : 'text-rose-600 bg-rose-50'
                                }`}>
                                {stat.trend} {stat.trendUp ? <ArrowUpRight size={10} /> : <ArrowDownRight size={10} />}
                            </span>
                        </div>
                        <div className="mt-4">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/60 rounded-3xl p-6 shadow-sm">
                    <div className="flex justify-between items-center mb-6">
                        <h3 className="font-black text-lg text-slate-800 dark:text-white uppercase tracking-tight">Urgent Case Alerts</h3>
                        <button className="text-blue-600 text-xs font-bold hover:underline">View All Alerts</button>
                    </div>
                    <div className="space-y-3">
                        {[
                            ...
                            <div key={i} className="flex items-center justify-between p-4 border border-slate-100 dark:border-slate-800 rounded-2xl hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors cursor-pointer group">
                                <div className="flex items-center gap-4">
                                    <div className={`h-2 w-2 rounded-full ${alert.priority === 'Critical' ? 'bg-rose-500 animate-pulse' : 'bg-amber-500'}`} />
                                    <div>
                                        <p className="text-sm font-bold text-slate-800 dark:text-slate-200">{alert.name} <span className="text-xs text-slate-400 ml-2">#{alert.id}</span></p>
                ...
                <div className="bg-gradient-to-br from-[#403081] to-[#5484B3] rounded-3xl p-6 text-white shadow-xl">
                    ...
                    <div className="space-y-6">
                        <div className="bg-white/10 p-4 rounded-2xl backdrop-blur-md">
                            <p className="text-xs text-white/70 uppercase font-bold tracking-widest">Total Placements</p>
