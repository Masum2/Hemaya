...
const ManageWidgets = () => {
    ...
    const confirmRemoveWidget = async () => {
        ...
        if (success) {
            ...
            // Client state filter to remove item instantly
            setAssignedWidgets(prev => prev.filter(item => item.Id !== widgetToDelete.Id));
    ...
    const handleAssignWidget = async (widget: WidgetItem) => {
        ...
        await assignSingleWidget(widget.WidgetComponentId, widget.WidgetType, () => {
            toast.success(`"${meta.name}" assigned successfully!`);
            
            // 1. Ready list theke item-ti bad jabe
            setReadyWidgets(prev => prev.filter(item => item.WidgetComponentId !== widget.WidgetComponentId));
            
            // 2. Assigned list-e dynamic structured item pass hobe
            const newAssignedItem: WidgetItem = {
                ...widget,
                Id: widget.Id || Date.now(), // Fallback generated ID if API doesn't return one directly
                IsInactive: false,
                IsMyStuff: false
            };
            setAssignedWidgets(prev => [...prev, newAssignedItem]);
        });
    };

    const handleAssignAllWidgets = async () => {
        await assignAllAvailableWidgets(readyWidgets, () => {
            // 1. Shob ready widget-ke update kore assigned view format-e convert kora
            const mappedToAssigned = readyWidgets.map((rw, index) => ({
                ...rw,
                Id: Date.now() + index, 
                IsInactive: false,
                IsMyStuff: false
            ...
            setAssignedWidgets(prev => [...prev, ...mappedToAssigned]);
            
            // 2. Ready stream purapuri empty hobe
            setReadyWidgets([]);
        });
    };
        ...
        <div className="min-h-screen p-6 bg-slate-50 text-slate-800 font-sans antialiased relative">
            <div className="max-w-7xl mx-auto space-y-6">
                ...
                <div className="flex gap-2 border-b border-slate-200">
                    <button
                        ...
                        className={`px-6 py-3 text-sm font-semibold transition-all relative ${activeTab === 'assigned' ? 'text-indigo-600' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        Assigned Widgets
                    ...
                    <button
                        ...
                        className={`px-6 py-3 text-sm font-semibold transition-all relative ${activeTab === 'ready' ? 'text-indigo-600' : 'text-slate-500 hover:text-slate-700'}`}
                    >
            ...
            {isModalOpen && widgetToDelete && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-x-hidden overflow-y-auto antialiased">
                    ...
                    <div className="relative w-full max-w-md bg-white rounded-2xl shadow-xl border border-slate-100 p-6 overflow-hidden transform transition-all z-10">
                        <button
                            disabled={actionLoading}
                        ...
                        <div className="flex flex-col items-center text-center mt-2">
                            ...
                            <h3 className="text-lg font-bold text-slate-900 mb-2">Remove Widget?</h3>
